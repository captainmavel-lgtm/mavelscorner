 ...
 *
 * After generating each MP3, the real file size (bytes) and duration (HH:MM:SS)
 * are saved into podcast-metadata.json in R2. The podcast feed reads this file
 * to populate accurate <enclosure length> and <itunes:duration> values.
 *
...
exports.handler = async function (event) {
  ...
  for (const post of posts) {
    ...
    /* ── CACHE CHECK: skip if already exists in R2 AND metadata is present ── */
    try {
      const exists = await checkR2FileExists(
        R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY,
        R2_BUCKET_NAME, fileName
      );
      if (exists && metadata[post.slug] && metadata[post.slug].size > 0) {
        console.log('Already exists with metadata, skipping:', fileName);
        results.push({ slug: post.slug, status: 'cached', url: publicUrl });
        continue;
      }

      // File exists in R2 but metadata is missing — fetch headers to get size
      if (exists && (!metadata[post.slug] || metadata[post.slug].size === 0)) {
        console.log('File exists but missing metadata, fetching size:', fileName);
        try {
          ...
          if (fileSize > 0) {
            // Download to calculate duration
            const mp3Buffer = await downloadR2File(
              R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY,
              R2_BUCKET_NAME, fileName
            );
            ...
            console.log('Backfilled metadata for:', post.slug, fileSize, duration);
            results.push({ slug: post.slug, status: 'metadata-backfilled', url: publicUrl });
            continue;
          }
        } catch (e) {
          console.warn('Could not backfill metadata for', fileName, e.message);
        }
      }
    } catch (e) {
    ...
    try {
      ...
      // Calculate and store real size and duration
      const fileSize = mp3Buffer.length;
      const duration = calculateMp3Duration(mp3Buffer);
      metadata[post.slug] = { size: fileSize, duration };
      metadataChanged = true;

      console.log('Generated and uploaded:', fileName, fileSize, 'bytes,', duration);
  ...
  if (metadataChanged) {
    try {
      ...
      await uploadToR2(
        R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY,
        R2_BUCKET_NAME, METADATA_FILE_NAME, metaBuffer, 'application/json'
      );
      console.log('Saved podcast-metadata.json to R2');
    } catch (e) {
...
/* ════════════════════════════════════════════════
   ...
   Reads ID3 header (if present) then scans MPEG
   frame headers to compute total duration.
════════════════════════════════════════════════ */
function calculateMp3Duration(buffer) {
  try {
    ...
    // MPEG bitrate table (layer 3, MPEG1)
    const bitrateTable = [0, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 0];
    // Sample rate table
    const sampleRateTable = [44100, 48000, 32000, 0];

    let totalFrames    = 0;
    let samplesPerFrame = 1152; // MPEG1 Layer3
    let sampleRate      = 44100;
    let foundFirst      = false;

    while (offset + 4 < buffer.length) {
      // Look for frame sync: 0xFF 0xEx or 0xFF 0xFx
      ...
      const mpegVersion  = (h1 >> 3) & 0x03; // 3 = MPEG1
      const layer        = (h1 >> 1) & 0x03; // 1 = Layer3
...
async function downloadR2File(accountId, accessKeyId, secretKey, bucket, key) {
  const res = await r2Request('GET', accountId, accessKeyId, secretKey, bucket, key, null, {});
  if (res.status !== 200) throw new Error(`R2 GET returned ${res.status}`);
  return res.body;
}
